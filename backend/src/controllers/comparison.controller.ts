...
export const getComparison = asyncHandler(async (req: Request, res: Response) => {
  const result = await comparisonService.getComparison(req.params.rfqId);
  res.json(apiResponse.success(result));
});

export const selectQuotation = asyncHandler(async (req: Request, res: Response) => {
  await comparisonService.selectQuotation(req.params.rfqId, req.body.quotationId, req.user.id);
